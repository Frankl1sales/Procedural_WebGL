...
async function main() {
  ...
  const textures = {
    defaultWhite: twgl.createTexture(gl, { src: [255, 255, 255, 255], premultiplyAlpha: false  }),
  };
  ...
  const numBanners = 10; // Número de banners
  const bannerDistance = 20; // Distância entre banners
  const bannerRowHeight = 5; // Altura entre fileiras
  const bannerTransforms = createTransforms(numBanners, bannerDistance, bannerRowHeight);
  ...
  const bannerParts = await loadObj(gl, baseHref, meshProgramInfo); // Passa meshProgramInfo
