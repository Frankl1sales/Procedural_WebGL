...
function createBannerTransforms(numBanners, distanceBetweenBanners, rowHeight) {
  const transforms = [];
  for (let row = 0; row < Math.ceil(numBanners / 5); row++) { // 5 banners por linha
    for (let i = 0; i < Math.min(5, numBanners - row * 5); i++) { // Ajusta para o número total de banners
      const zPosition = row * rowHeight; // Distância entre fileiras
      const xPosition = i * distanceBetweenBanners; // Distribui em X
      const scale = [1, 1, 1]; // Escala padrão
...
async function loadBanner(gl, baseHref, meshProgramInfo) {
  const bannerHref = 'assets/banner_blue.obj'; // Ajuste o caminho conforme necessário
...
async function loadObj(gl, baseHref, meshProgramInfo) {
  const bannerHref = 'assets/banner_blue.obj'; // Ajuste o caminho conforme necessário
  const response = await fetch(bannerHref);
  const text = await response.text();
  const obj = parseOBJ(text);
